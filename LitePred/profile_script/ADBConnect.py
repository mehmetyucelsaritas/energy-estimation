...
class ADBConnect:
    ...
    def run_cmd(self, cmd, use_su=False):
        if use_su:
            shell_cmd = f'adb -s {self.serial} shell su -c "{cmd}"'
        else:
            shell_cmd = f'adb -s {self.serial} shell "{cmd}"'
        results = subprocess.check_output(shell_cmd, shell=True).decode('utf-8')
        return results