...
from trainer import features as kernel_features
import scipy.stats
class Detector:
    ...
    def get_pred_list(self):
        self.predlists = {}
        model = NeuralNetwork(input_features=kernel_features[self.kernel_type])
        for device,predictor in self.predictors.items():
            model.load_state_dict(torch.load(predictor, map_location="cpu"))
