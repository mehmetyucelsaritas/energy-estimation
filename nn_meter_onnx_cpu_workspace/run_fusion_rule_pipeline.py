...
def main():
    ...
    # Force CPU-only profiling for Apple Silicon runs in this workspace.
    backend_config = builder_config.get_module("backend") or {}
    builder_config.set_module(backend_config, "backend")
    builder_config.set("EXECUTION_PROVIDERS", CPU_PROVIDER, "backend")
    builder_config.set("USE_COREML_EP", False, "backend")

    print(f"(2) Profile on backend {BACKEND!r} with provider {CPU_PROVIDER!r}…")
